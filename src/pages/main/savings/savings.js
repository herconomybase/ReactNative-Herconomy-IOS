...
const Savings = props => {
  ...
  const processData = async () => {
    try {
      ...
      console.log('User D is equal to', userD);
      let gql_token = await getData('gql_token');
      console.log('debug got here gql_toke', gql_token);
  ...
  const fetchData = async () => {
    try {
      ...
      let gql_token = await getData('gql_token');
      console.log('this na the user token!', gql_token);
      setFetching(true);
      if (!gql_user) return;
      let query = `query{
        ql_user : users(
      ...
      console.log('this is the request query we are sending o', res);
      ...
      setSavings({
        amount_saved: (chall_savings.amount_saved || 0) + (goals_saving.amount_saved || 0) + (user.wallet_balance || 0),
        avg_roi: (chall_savings.avg_roi || 0) + (goals_saving.avg_roi || 0),
      });
      // console.log('plan amount available', chall_savings.amount_saved);
      // console.log('goals amount available', goals_saving.amount_saved);
        ...
        {loading ? (
          ...
          <React.Fragment>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <TouchWrap onPress={() => props.navigation.navigate('GoalName')}>
                <Container
                  direction="row"
                  widthPercent="100%"
                  marginTop={3}
                    ...
                    <TouchWrap
                      ...
                      <Container
                        ...
                        <Container widthPercent="30%" verticalAlignment="center">
                          <ImageWrap url={item.image ? item.image : null} height={5} fit={'contain'} />
                        </Container>
